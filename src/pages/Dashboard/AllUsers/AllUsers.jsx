...
const AllUsers = () => {
    ...
    const handleMakeAdmin = user => {
        fetch(`https://tasty-trove-server.vercel.app/users/admin/${user._id}`, {
            method: 'PATCH'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handleDelete = user => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`https://tasty-trove-server.vercel.app/users/${user._id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(data => {
                    ...
                    <tbody>
                        {
                            users.map((user, index) => <tr key={user._id}>
                                ...
                                <td>{user.email}</td>
                                <td>{
                                    user.role === 'admin' ? 'admin' : <button onClick={() => handleMakeAdmin(user)} className="btn btn-ghost bg-[#bd8aff]">
